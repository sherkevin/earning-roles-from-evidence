#!/usr/bin/env python3
"""Prospective v2, development-only AppWorld producer -> consumer smoke.
...
CONFIG = ROOT / "configs/aamas2027/peer_judgment_smoke_v2.json"
NATIVE_PROMPT = prior.APP / "experiments/prompts/react_code_agent/instructions.txt"
...
def load_config() -> dict[str, Any]:
    ...
    if (cfg["version"] != 2
            or cfg["status"] != "PROSPECTIVE_DEVELOPMENT_AMENDMENT_ONLY"
            ...
            or cfg["native_task_ids_in_order"] != ["22cc237_2", "3c13f5a_2"]
            or any(type(caps.get(key)) is not int or caps[key] < 1
            ...
            or cfg["max_output_tokens_per_call"] != 2048
            or cfg["world_seed"] != 100):
        raise RuntimeError("Prospective v2 smoke contract differs from reviewed runner assumptions")
    for name in ("config", "runner_at_execution", "results"):
        path = ROOT / cfg["amends_v1"][name]
        if not path.exists() or prior.digest(path) != cfg["amends_v1"][name + "_sha256"]:
            raise RuntimeError("Preserved v1 evidence hash changed: " + name)
...
def api_guard(world: Any, phase: str, secrets: Secrets, raw: Path) -> list[dict[str, Any]]:
    """At the common requester boundary block task writes until action phase."""
    ...
    def checked(_app_name: str, _api_name: str, **kwargs: Any) -> Any:
        try:
            ...
            prior.log(raw, "api_denied", phase=phase, app=_app_name, api=_api_name,
        ...
        if phase in READ_PHASES and method != "get" and not credentials_only:
            prior.log(raw, "write_blocked", phase=phase, app=_app_name, api=_api_name,
                      method=method, api_path=path)
        ...
        try:
            ...
            audit.append(row)
            prior.log(raw, "public_api_call", **row)
    ...
    if phase in READ_PHASES:
        for verb in ("post", "put", "patch", "delete"):
            ...
            def blocked(url: str, *args: Any, _verb: str = verb, _old: Any = old, **kwargs: Any) -> Any:
                ...
                prior.log(raw, "write_blocked", phase=phase, method=_verb, api_path=url)
...
def render_native_prompt(world: Any, parser: Any, extra: str) -> list[dict[str, str]]:
    ...
    rendered = Template(NATIVE_PROMPT.read_text().lstrip()).render(
...
def episode(task_id: str, cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    if not manifest_path.exists():
        raise RuntimeError("Missing prospective v2 source snapshot")
    ...
    if (prior.digest(__file__) != manifest["runner_sha256"]
        ...
        raise RuntimeError("v2 runner/config differs from frozen execution snapshot")
    ...
    started = prior.mono()
    suffix = sha_text(f"peerjudgment-v2:{task_id}:{prior.digest(CONFIG)}")[:18]
    experiments = {"producer": "pj2_prod_" + suffix,
                   "consumer": "pj2_cons_" + suffix}
    ...
    apis: dict[str, Any] = {}
    state: dict[str, Any] = {"timeout_retry_used": False, "world_execute_count": 0,
    ...
    try:
        ...
        prior.log(raw, "episode_config", task_id=task_id, config_sha256=prior.digest(CONFIG),
                  script_sha256=prior.digest(__file__), native_prompt_sha256=prior.digest(NATIVE_PROMPT),
                  ...
                  amendment_version=cfg["version"], v1_results_sha256=cfg["amends_v1"]["results_sha256"],
        ...
        with AppWorld(task_id=task_id, experiment_name=experiments["producer"],
                      load_ground_truth=False, random_seed=cfg["world_seed"],
                      raise_on_extra_parameters=True) as producer:
            install_native_log_scrubber(producer, native_dirs["producer"], secrets)
            producer_audit = api_guard(producer, "producer", secrets, raw)
            producer_messages = render_native_prompt(producer, parser,
                "You are the independent PRODUCER. Inspect public task data only. "
                "Before proposing, execute at least one public task-data GET in this world; "
                "API documentation and authentication calls do not count. Every request row "
                "must cite an observed step that performed such a read. "
                "Do not make any task write or complete the task. Use only public APIs and "
                "ordinary Python computation; do not access files or runtime internals. "
                "When ready, emit exactly <proposal>{JSON}</proposal> outside any code fence. "
                "JSON keys: summary (string), requests (1-20 objects), uncertainties (list of strings). "
                "Each request has recipient_email (string), amount_usd (number), basis (string), "
                "source_steps (nonempty list of your actual public observation step numbers). "
                "Report uncertainty honestly. No passwords or access tokens in the proposal.")
            apis["producer"] = prior.RealAPI(raw, cfg["attempted_call_caps_per_episode"]["producer"])
            proposal_raw, producer_steps = phase_loop(
                producer, parser, apis["producer"], producer_messages, raw, "producer",
                PROPOSAL_TAG, cfg["max_output_tokens_per_call"], secrets, producer_audit,
                state, lambda value, steps: validate_proposal_v2(value, steps, secrets))
            proposal = proposal_raw
            if any(row["method"] not in ("get", "post") or
                   (row["method"] == "post" and not row["api_path"].endswith("/auth/token"))
                   for row in producer_audit):
                raise PermissionError("Producer audit contains task mutation")
            prior.save(episode_dir / "proposal.json", proposal)
            prior.log(raw, "producer_sealed", proposal_sha256=prior.digest(episode_dir / "proposal.json"),
                      public_steps=len(producer_steps), public_api_calls=len(producer_audit))
            producer.save()
        sanitize_native_logs(native_dirs["producer"], secrets)
        ...
        with AppWorld(task_id=task_id, experiment_name=experiments["consumer"],
            ...
            judgment_raw, review_steps = phase_loop(
                ...
                lambda value, steps: validate_judgment(value, secrets))
...
def run_all(cfg: dict[str, Any]) -> None:
    ...
    prior.save(root / "source_manifest.json", {
        ...
        "v1_runner_at_execution_sha256": cfg["amends_v1"]["runner_at_execution_sha256"],
        "v1_results_sha256": cfg["amends_v1"]["results_sha256"],
