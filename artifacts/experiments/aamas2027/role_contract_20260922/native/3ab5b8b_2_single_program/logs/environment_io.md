...
_r1_rows = _r1_pages(apis.spotify.show_song_library)
_r1_liked = _r1_pages(apis.spotify.show_liked_songs)
_r1_s = {sid for row in _r1_rows for sid in (row['song_id'] if 'song_id' == 'song_ids' else [row['song_id']])}
...
```
{"source": [4, 9, 15, 28, 87, 103, 110, 151, 199, 207, 208, 265, 294, 301], "predicate": [8, 9, 15, 17, 18, 19, 21, 24, 28, 29, 30, 32, 33, 35, 36, 44, 45, 48, 55, 58, 59, 80, 87, 97, 110, 112, 132, 143, 151, 163, 199, 202, 221, 223, 248, 259, 323], "selected": [9, 15, 28, 87, 110, 151, 199], "entity_type": "song"}
```
...
```python
import json
print(json.dumps(sorted(list(liked_library_song_ids))))
```
...
{"message":"The song is already downloaded."}
Song 15: Response status code is 422:
{"message":"The song is already downloaded."}
