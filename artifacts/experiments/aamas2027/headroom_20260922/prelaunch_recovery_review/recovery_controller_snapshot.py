...
def check():
    ...
    if LOG.exists():
        ...
        if any(e['event']=='recovery_outage_stop' for e in events):
            raise RuntimeError('Recovery outage is permanent; no second automatic recovery')
    gap=False
...
def main():
    ...
    context=('\nInfrastructure amendment: after observing the first27 attempts '
             ...
             'logged recovery amendment continued only the30 original unstarted IDs. '
