#!/usr/bin/env python3
"""Prospective, development-only AppWorld producer -> consumer observation smoke.
...
CONFIG = ROOT / "configs/aamas2027/peer_judgment_smoke_v1.json"
...
def load_config() -> dict[str, Any]:
    ...
    if (cfg["status"] != "PROSPECTIVE_DEVELOPMENT_OBSERVATION_SMOKE_ONLY"
            ...
            or cfg["native_task_ids_in_order"] != ["22cc237_1", "3c13f5a_1"]
        ...
        raise RuntimeError("Prospective smoke contract differs from reviewed runner assumptions")
...
def generate_with_retry(api: Any, messages: list[dict[str, str]], raw: Path,
    ...
    while True:
        try:
            ...
            if code not in (429, 503) or retries >= 2 or api.attempts >= api.max_attempts:
                raise
            delay = (1, 2)[retries]
            retries += 1
            prior.log(raw, "transport_backoff", seconds=delay, status=code, retry_index=retries)
            time.sleep(delay)
...
def phase_loop(world: Any, parser: Any, api: Any, messages: list[dict[str, str]],
               ...
               secrets: Secrets, audit: list[dict[str, Any]]) -> tuple[Any | None, list[dict[str, Any]]]:
    steps: list[dict[str, Any]] = []
    while api.attempts < api.max_attempts:
        answer = generate_with_retry(api, messages, raw, max_tokens)
...
def actual_use(proposal: dict[str, Any], action_calls: list[dict[str, Any]]) -> dict[str, Any]:
    writes = [row for row in action_calls if row["app"] == "venmo" and
              row["api"] == "create_payment_request" and row["ok"]]
...
def episode(task_id: str, cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    started = prior.mono()
    suffix = sha_text(f"peerjudgment-v1:{task_id}:{prior.digest(CONFIG)}")[:18]
    experiments = {"producer": "pj_prod_" + suffix,
                   "consumer": "pj_cons_" + suffix}
    ...
    try:
        ...
        with AppWorld(task_id=task_id, experiment_name=experiments["producer"],
            ...
            proposal = validate_proposal(proposal_raw, {row["step"] for row in producer_steps}, secrets)
        ...
        with AppWorld(task_id=task_id, experiment_name=experiments["consumer"],
            ...
            judgment_raw, review_steps = phase_loop(
                ...
                secrets, review_audit)
            judgment = validate_judgment(judgment_raw, secrets)
