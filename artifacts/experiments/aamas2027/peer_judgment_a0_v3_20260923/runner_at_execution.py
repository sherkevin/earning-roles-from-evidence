#!/usr/bin/env python3
"""Frozen A0 AppWorld peer-handoff diagnostic. `check` never calls an LLM.
...
def load_config() -> dict[str, Any]:
    ...
    if (cfg["version"] != 3 or cfg["status"] != "FROZEN_PROSPECTIVE_A0_DEVELOPMENT_DIAGNOSTIC_V3"
            ...
            or caps != {"producer": 8, "consumer_review": 5, "consumer_action": 7, "total": 20}
            or cfg["global_task_attempt_cap"] != 80
...
def generate_with_retry(api: prior.RealAPI, messages: list[dict[str, str]],
    ...
    while True:
        try:
            ...
            if (timed_out and not state["timeout_retry_used"] and
                    state["world_execute_count"] == 0 and api.attempts < api.max_attempts):
                state["timeout_retry_used"] = True
                prior.log(raw, "transport_backoff", retry_kind="one_pre_world_timeout",
                          seconds=2, charged_attempts=api.attempts)
                time.sleep(2)
                continue
            code = getattr(exc, "code", None)
            if code in (429, 503) and retries < 2 and api.attempts < api.max_attempts:
                delay = (1, 2)[retries]
                retries += 1
                prior.log(raw, "transport_backoff", retry_kind="service", status=code,
                          seconds=delay, retry_index=retries, charged_attempts=api.attempts)
                time.sleep(delay)
                continue
...
def episode(job: dict[str, Any], cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    suffix = sha_text("peer-a0-v3:" + task_id + ":" + config_hash)[:18]
