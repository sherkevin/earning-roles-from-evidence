...
if curl -s --max-time 5 "http://localhost:${VLLM_PORT}/v1/models" 2>/dev/null | grep -q "${SERVED_NAME}"; then
  ...
  CUDA_VISIBLE_DEVICES=${VLLM_GPU} nohup "${VENV_DIR}/bin/python3" -m vllm.entrypoints.openai.api_server \
      --model "${MODEL_DIR}" \
...
SAMPLES="/media/data3/dengkw/idea04/artifacts/round2_gpt41mini_fullval/run_20260414_135408/fixed_peer_calibrated/raw_inputs.jsonl"
...
# run sequentially to avoid GPU contention at batch size 4 × 2 methods
...
for method in single_agent edo_stage2_chain; do
  ...
  if [ ! -f "${OUT_ROOT}/${method}/metrics.json" ]; then
    log "  ${method}: TIMEOUT after 20 min; tail log:"
