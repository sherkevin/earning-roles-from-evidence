...
OUT=artifacts/external_baselines/reagent/r41c_w3_n5_$(date +%H%M%S)
mkdir -p "${OUT}"
echo "out: ${OUT}"
PYTHONUNBUFFERED=1 timeout 240 external_baselines/reagent/venv_reagent/bin/python -u \
...
echo ""
echo "=== PREDICTIONS ==="
cat "${OUT}/predictions.jsonl" 2>/dev/null | head -5
