...
def parse_sse(raw):
    ...
    """
    blocks = {}
    result = {"usage": {}, "content": [], "thinking_delta_count": 0}
    ...
    for frame in raw.replace("\r\n", "\n").split("\n\n"):
        ...
        elif kind == "content_block_delta" and event["delta"].get("type") == "thinking_delta":
            result["thinking_delta_count"] += 1
...
def call_api(out, stage_dir, stage, prompt, card):
    ...
    if "thinking" in card:
        request["thinking"] = card["thinking"]
    ...
    try:
        ...
        metadata.update({"usage": result.get("usage"), "returned_model": result.get("model"),
                         ...
                         "thinking_delta_count": result.get("thinking_delta_count")})
    ...
    if card.get("thinking", {}).get("type") == "disabled" and result.get("thinking_delta_count", 0):
        raise RuntimeError("Disabled thinking configuration did not take effect")
