#!/usr/bin/env python3
"""Prospective v3, development-only AppWorld producer -> consumer smoke.
...
CONFIG = ROOT / "configs/aamas2027/peer_judgment_smoke_v3.json"
...
def load_config() -> dict[str, Any]:
    ...
    if (cfg["version"] != 3
            or cfg["status"] != "PROSPECTIVE_RUNTIME_REPAIR_SMOKE_ONLY"
        ...
        raise RuntimeError("Prospective v3 smoke contract differs from reviewed runner assumptions")
    for name in ("config", "runner_at_execution", "results"):
        path = ROOT / cfg["amends_v2"][name]
        if not path.exists() or prior.digest(path) != cfg["amends_v2"][name + "_sha256"]:
            raise RuntimeError("Preserved v2 evidence hash changed: " + name)
...
def episode(task_id: str, cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    if not manifest_path.exists():
        raise RuntimeError("Missing prospective v3 source snapshot")
    ...
    if (prior.digest(__file__) != manifest["runner_sha256"]
        ...
        raise RuntimeError("v3 runner/config differs from frozen execution snapshot")
    ...
    started = prior.mono()
    suffix = sha_text(f"peerjudgment-v3:{task_id}:{prior.digest(CONFIG)}")[:18]
    experiments = {"producer": "pj3_prod_" + suffix,
                   "consumer": "pj3_cons_" + suffix}
    ...
    try:
        ...
        prior.log(raw, "episode_config", task_id=task_id, config_sha256=prior.digest(CONFIG),
                  ...
                  amendment_version=cfg["version"], v2_results_sha256=cfg["amends_v2"]["results_sha256"],
        ...
        with AppWorld(task_id=task_id, experiment_name=experiments["producer"],
            ...
            producer_audit = api_guard(producer, "producer", secrets, raw, deferred_events)
...
def run_all(cfg: dict[str, Any]) -> None:
    ...
    prior.save(root / "source_manifest.json", {
        ...
        "v2_runner_at_execution_sha256": cfg["amends_v2"]["runner_at_execution_sha256"],
        "v2_results_sha256": cfg["amends_v2"]["results_sha256"],
