...
def write_cr2_repair(workspace: Path) -> None:
    """Apply a deterministic, behavior-preserving style repair fixture."""
    (workspace / "test_helpers.py").write_text(
...
def apply_fixture_candidate(task: str, peer: str, workspace: Path) -> None:
    if peer == "peer_02_repair":
        if task == "CR2_style_enforce":
            write_cr2_repair(workspace)
...
def choose_peer(condition: str, scores: dict[str, float], episode: int) -> str:
    if condition == "random":
        return PEERS[episode % len(PEERS)]
    if condition == "static_best":
        ...
        return "peer_02_repair"
    return max(PEERS, key=lambda p: (scores.get(p, 0.0), p))
...
def secure_grade_candidate(task: str, candidate_dir: Path, seed: int,
    ...
    if task == "CR2_style_enforce":
        source = candidate_workspace / "test_helpers.py"
        if source.exists():
            shutil.copy2(source, eval_workspace / source.name)
    elif task == "DIST1_queue_race":
...
def run_episode(task: str, condition: str, episode: int, runs_dir: Path,
                records: list[dict[str, Any]], scores: dict[str, float],
                ledger: PeerRoleLedger) -> None:
    seed = episode
    task_dir = TEAMBENCH / "tasks"
    ledger.record_task_start(task, episode)
    candidate_runs: dict[str, tuple[str, Path, str]] = {}
    candidate_info: list[dict[str, Any]] = []

    for peer in PEERS:
        _, candidate_dir, _ = setup_run(task, str(task_dir), str(runs_dir), seed=seed)
        candidate_path = Path(candidate_dir)
        apply_fixture_candidate(task, peer, candidate_path / "workspace")
        write_attestation(candidate_path)
        artifact_hash = tree_digest(candidate_path / "workspace")
        candidate_runs[peer] = (peer, candidate_path, artifact_hash)
    ...
    chosen = choose_peer(condition, scores, episode)
    selection = PeerSelection(
        ...
        propensity=1.0 / len(PEERS) if condition == "random" else 1.0,
    )
    ledger.record_selection(selection)
    _, chosen_dir, artifact_hash = candidate_runs[chosen]
    delivery = Delivery(
    ...
    before_grade_hash = tree_digest(chosen_dir / "workspace")
    score, eval_dir, grade_audit = secure_grade_candidate(
        task, chosen_dir, seed, runs_dir
    )
    after_grade_hash = tree_digest(chosen_dir / "workspace")
    ...
    for task in TASKS:
        for condition in CONDITIONS:
            ...
            for episode in range(2):
                run_episode(task, condition, episode, runs_dir, records, scores, ledger)
