#!/usr/bin/env python3
"""Prospective v4, development-only AppWorld producer -> consumer smoke.
...
CONFIG = ROOT / "configs/aamas2027/peer_judgment_smoke_v4.json"
...
def load_config() -> dict[str, Any]:
    ...
    if (cfg["version"] != 4
            or cfg["status"] != "PROSPECTIVE_COMPACT_PROMPT_SMOKE_ONLY"
        ...
        raise RuntimeError("Prospective v4 smoke contract differs from reviewed runner assumptions")
    for name in ("config", "runner_at_execution", "results"):
        path = ROOT / cfg["amends_v3"][name]
        if not path.exists() or prior.digest(path) != cfg["amends_v3"][name + "_sha256"]:
            raise RuntimeError("Preserved v3 evidence hash changed: " + name)
...
def episode(task_id: str, cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    if not manifest_path.exists():
        raise RuntimeError("Missing prospective v4 source snapshot")
    ...
    if (prior.digest(__file__) != manifest["runner_sha256"]
        ...
        raise RuntimeError("v4 runner/config differs from frozen execution snapshot")
    ...
    started = prior.mono()
    suffix = sha_text(f"peerjudgment-v4:{task_id}:{prior.digest(CONFIG)}")[:18]
    experiments = {"producer": "pj4_prod_" + suffix,
                   "consumer": "pj4_cons_" + suffix}
    ...
    try:
        ...
        prior.log(raw, "episode_config", task_id=task_id, config_sha256=prior.digest(CONFIG),
                  script_sha256=prior.digest(__file__), prompt_mode="compact_v4",
                  ...
                  amendment_version=cfg["version"], v3_results_sha256=cfg["amends_v3"]["results_sha256"],
...
def run_all(cfg: dict[str, Any]) -> None:
    ...
    prior.save(root / "source_manifest.json", {
        ...
        "v3_runner_at_execution_sha256": cfg["amends_v3"]["runner_at_execution_sha256"],
        "v3_results_sha256": cfg["amends_v3"]["results_sha256"],
