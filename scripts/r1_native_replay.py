...
OUT = ROOT / 'artifacts/experiments/aamas2027/role_contract_20260922'
...
def execute(task_id, arm):
    ...
    experiment='r1_'+hashlib.sha256(job_id.encode()).hexdigest()[:20]
        ...
        with AppWorld(task_id=task_id,experiment_name=experiment,
            ...
            for i, step in enumerate(trace['steps']):
                ...
                if i==index and arm!='original':
                    ...
                    insertion=helper_code(endpoint,field,hypotheses)
...
def batch():
    ...
    equality=all(by[(t['task_id'],'qualified')]['actual_consumed_selection']==
                 by[(t['task_id'],'single_program')]['actual_consumed_selection']
                 and by[(t['task_id'],'qualified')]['official_success']==
                 by[(t['task_id'],'single_program')]['official_success']
                 for t in manifest['tasks'])
    summary={'status':'COMPLETE','new_policy_llm_calls':0,'results':results,
        'single_program_explains_qualified':equality,
        'source_version_space_size':len(manifest['source_contract']['hypotheses']),
        'decision':'NO_INCREMENTAL_MECHANISM_EVIDENCE' if equality else 'INVESTIGATE_DIFFERENCE',
        'boundary':'Post-hoc native replay diagnosis, not autonomous repair or held-out agent performance.'}
...
if __name__=='__main__':
    ...
    parser.add_argument('--task'); parser.add_argument('--arm',choices=['original','qualified','single_program'])
