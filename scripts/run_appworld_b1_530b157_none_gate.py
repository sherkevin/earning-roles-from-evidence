#!/usr/bin/env python3
"""Run the frozen 530b157 no-experience dynamic-range gate on Mac."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--experiment-name",
        default="aamas_b1_qwen38_530b157_none_20260921",
    )
    ...
    contract_path = artifact_dir / "capability_differentiation_530b157_contract_v3.json"
    contract = load_json(contract_path)
    assert contract["benchmark"]["target_tasks"] == TASKS
    assert contract["backbone"]["model"] == args.model
    assert contract["pre_exposure_dynamic_range_gate"]["condition"] == "control_none"
    ...
    config = {
        ...
        "model_config": {
            ...
            "temperature": contract["backbone"]["temperature"],
        ...
        "max_steps": contract["backbone"]["max_steps"],
        "log_lm_calls": True,
    ...
    agent.language_model.generation_kwargs["max_completion_tokens"] = contract["backbone"][
    ...
    if successes == 3:
        decision = "CEILING_STOP_BEFORE_ACQUISITION"
    elif successes == 0 and mean_fraction <= 0.30 and none_completed:
        decision = "FLOOR_STOP_BEFORE_ACQUISITION"
    else:
        decision = "USABLE_PROCEED_TO_ACQUISITION"
    ...
    result = {
        ...
        "runtime": {
            ...
            "max_steps": contract["backbone"]["max_steps"],
            "max_output_tokens_per_turn": contract["backbone"][
                "max_output_tokens_per_turn"
            ],
            "temperature": contract["backbone"]["temperature"],
        },
        ...
        "claim_boundary": (
            "Pre-exposure development dynamic-range gate only. "
            "No donor acquisition or capability-difference treatment has run."
        ),
    }

    out_path = artifact_dir / "qwen38_530b157_none_gate_result.json"
    out_path.write_text(
        json.dumps(result, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    print("B1_530B157_NONE_GATE_RESULT")
