...
\begin{algorithm}[t]
\caption{TCPB execution loop (Stage-1 prototype, single sample). Notation matches \S\ref{sec:methodology}; deterministic instantiations of $\phi$ and $b_i(j)$ are in \texttt{artifacts/edo\_lite\_executable\_spec.md} \S 1.2 / \S 2.2.}
\label{alg:tcpb-loop}
\small
\begin{algorithmic}[1]
...
\end{enumerate}
The deterministic instantiations of $\phi(z)$, the utility estimators $U^{self}, U^{out}$, and the persona update rule for this prototype scope are given in \texttt{artifacts/edo\_lite\_executable\_spec.md}; that note is the canonical companion specification for everything in this box and is referenced by Sections~\ref{sec:methodology}, \ref{sec:experiments}, and \ref{sec:limitations} without restating its content. Stage-2 work will progressively replace each of (i)--(v) with the full EDO mechanism; none of those replacements are claimed by the current submission.
\end{minipage}}
\medskip
...
Our current reproducible evidence comes from the Stage-1 TCPB prototype on HotpotQA. The main benchmark slice is a fixed set of 200 HotpotQA validation questions exported to \fpath{artifacts/seed/hotpotqa_validation_200.jsonl}. Existing runs compare \texttt{fixed\_peer\_calibrated}, \texttt{fixed\_self\_calibrated}, \texttt{fixed\_self\_claim}, \texttt{fixed\_static\_roles}, and centralized baselines under shared inputs and logged execution traces.
...
\begin{itemize}
  \item \textbf{Benchmark:} HotpotQA \citep{yang2018hotpotqa} distractor validation subset, 200 examples (\fpath{artifacts/seed/hotpotqa_validation_200.jsonl}). MuSiQue \citep{trivedi2022musique} and 2WikiMultiHop \citep{ho20202wikimultihop} are reserved for the Stage-2 EDO benchmark transfer (\S 4.4 E5).
  \item \textbf{Model:} \texttt{gpt-4.1-mini} (OpenAI-compatible via \fpath{https://kuaipao.ai/v1}), temperature 0, chain topology unless otherwise noted. \texttt{model\_resolved\_runtime}: \texttt{gpt-4.1-mini} confirmed in all canonical run notes.
  \item \textbf{Canonical run:} \fpath{artifacts/round2_gpt41mini/run_20260414_115739/} --- contains \texttt{fixed\_peer\_calibrated}, \texttt{fixed\_static\_roles}, and \texttt{fixed\_self\_claim} subdirectories, all validated \texttt{[OK]}. Metrics consolidated in \fpath{artifacts/round2_gpt41mini/round2_gpt41mini_main_table.csv}.
  \item \textbf{Current runtime:} \fpath{workspace/idea04_core/runner.py}, \fpath{methods.py}, \fpath{contracts.py}.
  ...
  \item \textbf{Note on prior GLM results:} All runs under \fpath{artifacts/round1/} and \fpath{artifacts/round1_star/} used \texttt{glm-4-flash} as the actual runtime backbone. These are archived as Stage-1 guidance-only artifacts. They established the direction of the research but are not the final paper evidence.
\end{itemize}

\subsection{Current main findings from the restricted instantiation}

The canonical round2 \texttt{gpt-4.1-mini} results ($n=200$, chain, run \fpath{run_20260414_115739}) support four main observations, summarised in Table~\ref{tab:main-results}.
...
\bottomrule
\end{tabular}
\caption{Stage-1 results on HotpotQA chain-200 with \texttt{gpt-4.1-mini} (canonical run \fpath{round2_gpt41mini/run_20260414_115739}). MHC = mean handoff count; Tokens = mean API tokens per sample. All three methods achieve premature-accept rate $\mathrm{PAR}=0.0$.}
...
\bottomrule
\end{tabular}
\caption{Mechanism ablations, HotpotQA chain-200, \texttt{glm-4-flash}. Disabling TCPB or the decomposer gate exactly matches the refreshed baseline; enlarging the evidence window recovers $+2.2$ F1. PAR $=0$ for all variants. Source: \fpath{round1_ablation_\{baseline,evidence,notcpb,nogate\}/}.}
\label{tab:ablations}
\end{table}
...
\end{figure} The gap between the best and worst Stage-1 method is 2.6 F1 points --- non-trivial at $n=200$ but not yet confirmed with paired statistics. \texttt{fixed\_peer\_calibrated}/TCPB should \emph{not} be described as the strongest Stage-1 method on the strong backbone. \texttt{fixed\_self\_claim} currently leads F1; \texttt{fixed\_static\_roles} is the most cost-efficient (4{,}663 vs.\ 6{,}414 API tokens per sample). $N{=}7405$ fullval for all three methods is required for tight confidence intervals. As of this draft, \texttt{peer\_calibrated} fullval is completed and valid (F1=0.7703, $n{=}7405$, \fpath{artifacts/round2_gpt41mini_fullval/}); \texttt{static\_roles} and \texttt{self\_claim} fullval runs were invalidated by a provider runtime integrity failure (model drift mid-run) and are pending rerun under the new fail-fast guard. The three-method ordering above is therefore supported by chain-200 evidence ($n{=}200$) only and should not be presented as a finalized fullval ranking.
...
The current artifact directories, tables, and ablations should remain in the paper, but their framing must change. They are evidence for the restricted instantiation. Canonical source: \fpath{artifacts/round2_gpt41mini/} (backbone: \texttt{gpt-4.1-mini}); GLM-backbone results under \fpath{artifacts/round1/} are archived as Stage-1 guidance-only.

Additional GLM-backbone evidence reinforces this reading. Mechanism ablations (Table~\ref{tab:ablations}) confirm that on the weak backbone the local answer-generation bottleneck dominates the routing-mechanism contribution; weight-sensitivity sweeps ($\pm 2\times$, $<0.003$ pp F1 change) rule out fine-tuned weights; and a static-path overlap analysis (\texttt{peer\_calibrated} reproducing ${\sim}52\%$ of static routes) shows stable route structure can form without central control. These narrow the Stage-2 target toward task trees, recursive audit, persona tags, and sparse-graph organization rather than further fixed-route weight tuning.
...
A fourth limitation concerns backbone-sensitive method ordering. On our current 200-sample HotpotQA chain slice, the relative ranking of Stage-1 delegation methods inverts between \texttt{glm-4-flash} and \texttt{gpt-4.1-mini}. On the stronger backbone, \texttt{fixed\_self\_claim} leads F1, while \texttt{fixed\_peer\_calibrated} trails. This indicates that the TCPB peer-calibration overhead (additional LLM calls for belief updates) does not yield F1 gains when the underlying model is already strong enough to self-evaluate accurately. This result is theoretically important: it predicts that the full EDO organizational advantage will be most visible on harder tasks, weaker individual agents, or richer topologies where single-agent self-assessment breaks down. Reporting Stage-1 results on only one backbone would conceal this sensitivity; we report both for transparency. A related data integrity note: one fullval run (\fpath{artifacts/round2_gpt41mini_fullval/run_20260414_135408/}) experienced provider-side model drift that silently substituted an unsupported model for two of the three methods; the affected \texttt{static\_roles} and \texttt{self\_claim} fullval results are being rerun under a new fail-fast runtime guard (\texttt{ModelDriftError} in \fpath{workspace/idea04_core/llm_client.py}). The \texttt{peer\_calibrated} fullval result (F1=0.7703) remains valid. This section will be updated once the repaired reruns land.
