...
\begin{figure}[t]
\centering
\includegraphics[width=0.96\columnwidth]{figure1_v1.png}
...
\begin{enumerate}[leftmargin=*,itemsep=0pt,topsep=2pt]
  ...
  \item We specify \textbf{EDO-Frame} as a falsifiable framework around sparse local graphs, recursive \texttt{do / outsource / split} decisions, governed note-board memory, tag-gated tool access, recursive upstream audits, and value-induced personality tags.
  ...
  \item Using current HotpotQA evidence, we report claim-bounded measurements: TCPB is F1-dominated by \texttt{self\_claim} on a capable API backbone, while a local \texttt{Phi-4-mini} gain weakens under multi-seed and transfer checks.
\end{enumerate}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.96\textwidth]{figure2_v2.png}
\caption{Two-column EDO-Frame pipeline sketch. A task packet becomes local agent views; agents interact through sparse delegation, governed note-board memory, tag-gated tool selection, action envelopes, and audit events. The figure is a mechanism overview, not a reported result.}
...
These components exist as a dependency-light local prototype under the anonymous code supplement. Unit tests pass locally and remotely; memory writes carry source references, selector outputs log score components, and events serialize to JSONL-compatible dictionaries. Transparent-bridge diagnostics first showed no metric movement; later consumption-layer diagnostics make the switches affect answers, but the default memory/tool configuration underperforms the simpler local chain and the no-memory variant. Thus this layer remains \emph{method infrastructure and negative ablation evidence}, not a positive component-level performance claim.
...
These experiments do \emph{not} establish the full EDO theory. What they establish is narrower but still useful:
\begin{itemize}
  \item strong-backbone TCPB is not a performance winner, so the Stage-1 result is a negative calibration measurement;
  \item a local \texttt{Phi-4-mini} chain gain exists on one HotpotQA slice but weakens under multi-seed and transfer checks;
  \item mechanism logs expose path length, audit rows, entropy, and memory/tool events, but the observed routing is deterministic and memory/tool switches are not performance-positive.
\end{itemize}
...
\begin{itemize}[leftmargin=*,itemsep=0pt,topsep=2pt]
  \item \textbf{Benchmark:} HotpotQA \citep{yang2018hotpotqa} distractor validation subset, 200 examples (versioned slice in the anonymous supplement). A no-paid MuSiQue \citep{trivedi2022musique} transfer check is negative; 2WikiMultiHop \citep{ho20202wikimultihop} remains gated on official local data.
...
\bottomrule
\end{tabular}
\caption{No-paid local open-weight diagnostic on the same HotpotQA chain-200 slice with \texttt{Phi-4-mini}. The chain improves F1 by +7.93 points over \texttt{single\_agent}; a 10,000-resample paired bootstrap gives a single-seed 95\% CI of [+1.52, +14.39] F1 points and sign-test $p{=}0.0186$. EM is positive but not significant; token cost is 2.33$\times$ higher; follow-up pooling weakens this into a candidate signal.}
\label{tab:local-phi4}
\end{table}

\paragraph{Finding 1: Backbone capability changes the value of organization.} All three Stage-1 methods improve by $+15$ to $+20$ F1 points when switching from \texttt{glm-4-flash} to \texttt{gpt-4.1-mini} on the same 200-sample slice and identical routing code. On the capable API backbone, routing is secondary to generation quality; on one local \texttt{Phi-4-mini} slice, the chain produces a positive F1 gain but pays a large token-cost premium (Table~\ref{tab:local-phi4}). Later multi-seed pooling crosses zero, while MuSiQue transfer and a smaller SmolLM3 control are negative.

\paragraph{Finding 2: Method ordering is backbone-sensitive and must not be over-interpreted from a single backbone.} On \texttt{glm-4-flash}, the ordering was \texttt{static\_roles} $>$ \texttt{peer\_calibrated} $\approx$ \texttt{self\_claim}. On \texttt{gpt-4.1-mini}, it inverts: \texttt{self\_claim} (0.7641 F1) $>$ \texttt{static\_roles} (0.7454) $>$ \texttt{peer\_calibrated} (0.7381). On one \texttt{Phi-4-mini} slice, the local Stage-2 chain beats \texttt{single\_agent} in F1 but not in cost-normalized F1; on MuSiQue and SmolLM3 controls it loses. This supports a capability-boundary reading rather than a universal ``more agents is better'' claim.

TCPB should \emph{not} be described as the strongest Stage-1 method on the strong backbone: \texttt{self\_claim} leads F1, while \texttt{static\_roles} is most cost-efficient. Full-validation reruns, external baselines, and stronger robustness evidence are still required before a finalized ranking.
...
\paragraph{Finding 4: delegation overhead helps only under specific capability and cost regimes.} \texttt{peer\_calibrated} matches \texttt{self\_claim}'s token budget on the strong backbone but with lower F1; the local chain improves one \texttt{Phi-4-mini} slice while more than doubling token use. Mechanism logs are now measurable (path length, audit rows, terminal entropy, memory/tool events), but they bound rather than strengthen the claim: summarized chain routing has zero terminal-agent entropy, and \texttt{no\_memory} outperforms the full memory/tool variant. EDO therefore tests when organization may compensate for individual model limits, not that delegation is uniformly Pareto-superior.

\section{Conclusion}

We proposed \textbf{EDO}, treating multi-agent language systems as organizations formed via delegation and value-based updating. Current evidence is bounded and mixed; the contribution is therefore a falsifiable framework and audited evidence path, not a universal performance claim.

\section*{Limitations}
\label{sec:limitations}

We explicitly acknowledge seven scope limitations of the present submission. (1) The current runtime implements only the restricted Stage-1 TCPB prototype: fixed topologies, role-prior nodes, linear packet passing, deterministic accept-or-forward routing, and terminal-only outcome updates. It therefore cannot yet establish the strongest claims of the full EDO framework --- recursive split value, local audit superiority, or personality-tag emergence from near-homogeneous initialization. (2) Some current routing improvements remain partially entangled with hand-designed safety priors and answer-generation constraints; the present evidence is a capability-boundary probe, not proof that local interaction alone suffices for rich organizational emergence. (3) Backbone-sensitive method ordering: rankings differ across \texttt{gpt-4.1-mini}, \texttt{Phi-4-mini}, SmolLM3-3B, MuSiQue transfer, and earlier weak-backbone diagnostics; follow-up checks are mixed or negative rather than confirmatory. (4) Statistical evidence is still incomplete: Table~\ref{tab:local-phi4} includes a single-seed paired-bootstrap CI, but multi-seed pooling crosses zero and the paper still lacks paired statistics for all headline comparisons. (5) \textbf{Cross-endpoint comparability is an open variable}: a provider-side data-integrity event during this submission cycle led to a deliberate switch of the upstream OpenAI-compatible API channel serving the strong backbone, with affected reruns documented in the supplementary appendix. (6) \textbf{Delivered-system Pareto-domination}: the delivered TCPB Stage-1 instance is Pareto-dominated by \texttt{fixed\_self\_claim} on F1 at equal token cost on the chain-200 HotpotQA slice with \texttt{gpt-4.1-mini}; the positive local-chain result is not Pareto-dominant because it costs 2.33$\times$ more tokens and does not survive all robustness checks. (7) \textbf{Demographic and societal scope}: the main benchmark used (HotpotQA) is sourced exclusively from English Wikipedia; the limited MuSiQue transfer check is negative and does not establish multilingual, low-resource, or culturally diverse generalisation.
