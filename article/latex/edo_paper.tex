...
\subsection{Three primitive actions}

The EDO action space is intentionally minimal but organizationally expressive:
\begin{itemize}
  \item \texttt{do\_self}: solve the task locally.
  \item \texttt{outsource(neighbor)}: pass the task to the locally best visible neighbor.
  \item \texttt{split(subtasks)}: decompose the task into child tasks that each re-enter the same policy.
\end{itemize}
...
In addition, two GLM-backbone observations remain valid as mechanistic context: (i) the weight-sensitivity ablations (\fpath{artifacts/round1_v3_weight_sensitivity.md}) confirm that outcome-based calibration and conservative routing structure each contribute measurable signal; and (ii) the static-path overlap analysis (\texttt{peer\_calibrated} reproducing ${\sim}52\%$ of static routes) shows stable route structure can form without a central controller. The residual error budget appears increasingly attributable to the local answer-generation layer rather than to misrouting, narrowing the Stage-2 target toward task trees, recursive audit, persona tags, and sparse-graph organization.
