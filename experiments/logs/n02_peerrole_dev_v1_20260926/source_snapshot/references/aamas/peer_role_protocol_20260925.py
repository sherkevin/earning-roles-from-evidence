...
class LaterAssignment:
    ...
    def __post_init__(self) -> None:
        ...
        if not (0.0 < float(self.decision_propensity) <= 1.0):
            raise ValueError("decision_propensity must be in (0, 1]")
...
class PeerRoleLedger:
    ...
    def record_selection(self, selection: PeerSelection) -> None:
        ...
        assignments = [
            ...
            if (assignment.task_id, assignment.task_index) == key
        ]
        if len(assignments) > 1:
            raise ValueError("one later assignment is allowed per task episode")
        if assignments and assignments[0].agent_id != selection.chosen_peer_id:
            raise ValueError("selection must consume the recorded later assignment")
        if assignments and assignments[0].decision_propensity != selection.propensity:
            raise ValueError("selection must preserve the assignment propensity")
    ...
    def record_task_start(self, task_id: str, task_index: int) -> None:
        ...
        if self.require_selection and not any(
            (selection.task_id, selection.task_index) == key
            for selection in self.selections.values()
        ):
            raise ValueError("strict ledger requires selection before task start")
    ...
    def record_evidence_update(self, evidence: RoleEvidenceUpdate) -> None:
        ...
        if any(
            existing.judgment_id == evidence.judgment_id
            or existing.action_id == evidence.action_id
            for existing in self.evidence.values()
        ):
            raise ValueError("one role evidence update is allowed per judgment/action")
    ...
    def record_assignment(self, assignment: LaterAssignment) -> None:
        ...
        key = (assignment.task_id, assignment.task_index)
        if any(
            (existing.task_id, existing.task_index) == key
        ...
        if any(
            ...
            raise ValueError("assignment must be recorded before the assigned peer is selected")
