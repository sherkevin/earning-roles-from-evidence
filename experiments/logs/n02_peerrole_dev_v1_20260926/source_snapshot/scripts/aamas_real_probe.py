...
AMENDMENT = ROOT / "configs/aamas2027/acquisition_probe_v1_transport_amendment.json"


def amendment():
    return json.loads(AMENDMENT.read_text()) if AMENDMENT.exists() else {}


def acquisition_id(family):
    return amendment().get('acquisition_source_jobs',{}).get(family,'acquire_'+family)


def infrastructure_valid(row):
    if not row or not row['scorer_available'] or row['runtime_error'] or row['stop_reason']=='transport_failure':
        return False
    if not row['api_errors']:
        return True
    # The pre-validation amendment permits recovered rate/service errors, with
    # all unknown usage retained. No other infrastructure failure is waived.
    return bool(amendment() and row.get('error_codes') and
                set(row['error_codes']) <= {429,503})


def read_episode_result(path):
    path=Path(path)
    rescored=path.with_name('rescored_result.json')
    return json.loads((rescored if rescored.exists() else path).read_text())
...
class RealAPI:
    ...
    def generate(self, messages, max_tokens=2048):
        ...
        try:
            ...
            self.errors += 1
            self.error_codes.append(getattr(error,'code',type(error).__name__))
...
def episode(job):
    ...
    # Upstream applies `if "memory" in path` to detect SQLite memory storage.
    # Scientific arm labels must not enter the physical database path.
    experiment = "aamas_acq_20260922_" + hashlib.sha256(job['id'].encode()).hexdigest()[:16]
    ...
    log(raw, "episode_config", job=job, contract_sha256=digest(CONTRACT),
        transport_amendment_sha256=digest(AMENDMENT) if AMENDMENT.exists() else None,
        ...
        with AppWorld(task_id=job["task_id"], experiment_name=experiment,
            ...
            for step in range(contract["runtime"]["max_attempted_calls"]):
                try:
                    ...
                    repair=amendment()
                    retryable=getattr(error,'code',None) in [429,503]
                    if repair and retryable and consecutive_errors < 3:
                        delay=repair['retry_backoff_seconds'][consecutive_errors-1]
                        log(raw,'transport_backoff',seconds=delay,error_code=error.code)
                        time.sleep(delay)
                        continue
                    if (repair and retryable) or consecutive_errors >= 2:
...
def induce():
    ...
    for family in contract["selection"]["train_families"]:
        source_id=acquisition_id(family)
        source = OUT / "episodes" / source_id
        target = OUT / "induction" / (family if source_id=='acquire_'+family else family+'_retry1')
        ...
        error = None
        try:
            if not infrastructure_valid(outcome):
            ...
            candidate = None
        record = {"family": family, "source_task": family + "_1", "source_job_id": source_id,
...
def run_jobs(jobs):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=amendment().get('task_parallelism',2)) as pool:
...
def stage(name):
    ...
    if name == "acquire":
        ...
        induce()
    elif name == "repair_acquisition":
        repair=amendment()
        if not repair:
            raise RuntimeError('Missing prospective transport amendment')
        library_path=OUT/'memory_library.json'
        old_library=OUT/'memory_library_before_transport_repair.json'
        if library_path.exists() and not old_library.exists():
            old_library.write_bytes(library_path.read_bytes())
        run_jobs(repair['additional_jobs'])
        induce()
    elif name == "validate":
        if not analyze()['acquisition_infrastructure_valid']:
            raise RuntimeError('Acquisition infrastructure incomplete; do not launch validation')
...
def analyze():
    contract = json.loads(CONTRACT.read_text())
    rows = [read_episode_result(p) for p in sorted((OUT / 'episodes').glob('*/result.json'))]
    ...
    for family in contract['selection']['train_families']:
        ...
        if empty and memory:
            pairs.append({'family':family,'empty':empty['official_success'],'memory':memory['official_success'],
                          ...
                          'runtime_valid':infrastructure_valid(empty) and infrastructure_valid(memory)})
    nulls=[]
    for family in contract['selection']['null_families']:
        ...
        if first and second:
            nulls.append({'family':family,'success_flip':first['official_success']!=second['official_success'],
                          'runtime_valid':infrastructure_valid(first) and infrastructure_valid(second)})
    wins=sum(p['difference']>0 for p in pairs);losses=sum(p['difference']<0 for p in pairs)
    acquisition_valid=all(infrastructure_valid(by_id.get(acquisition_id(f))) for f in contract['selection']['train_families'])
...
def main():
    ...
    parser.add_argument('command',choices=['episode','acquire','repair_acquisition','validate','dev','analyze'])
