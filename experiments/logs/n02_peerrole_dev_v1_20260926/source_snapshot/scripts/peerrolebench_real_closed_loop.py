...
def call_api(out, stage_dir, stage, prompt, card):
    ...
    request = {"model": card["model"], "messages": [{"role": "user", "content": prompt}],
               "temperature": card["temperature"], "max_tokens": card["max_tokens"][stage], "stream": False}
    request_path = stage_dir / (stage + "_request.json")
    save(request_path, request)
    response_path = stage_dir / (stage + "_response.json")
    command = ["curl", "--silent", "--show-error", "--fail-with-body", "--noproxy", "*",
               "--connect-timeout", "15", "--max-time", str(card["request_timeout_seconds"]),
               "--config", "-", "--data-binary", "@" + str(request_path),
               "--output", str(response_path), "--write-out", "%{http_code}", provider["base"] + "/v1/messages"]
    ...
    metadata = {"elapsed_seconds": time.monotonic() - started, "http_status": proc.stdout,
...
def prepare(out):
    ...
    metadata = {"created_utc": now(), "card": card, "source_hashes": source_hashes,
