...
> 2026-09-26证据修正：[ADR0019](../user/decisions/0019-separate-judgment-from-responsibility-outcomes.md)
> 与[N02报告](n02_real_closed_loop_review_20260926.md)表明，下面use/repair/redo的
> 1/0.5/0示例只能作为原始判断编码，不能作为已验证producer正确性或团队效用。
> 正常集成可能被误报repair，消费成功也可能掩盖交付错误。因此第2节以平均该编码
> 为目标的写法不是后续实验的已冻结目标；新卡须分别确定独立结果与完整成本。
> 延迟约束也须测反馈到服务生效，不能仅报U的一次计算时间。原文保留为建模历史，
> ADR0013最小概念边界不变，peer个人经验与公开证据的具体实现仍待审查。
