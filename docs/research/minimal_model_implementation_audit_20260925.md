...
1. `j_t` 应由任务 owner/consumer 规则产生并可追踪；若 owner 已在 `x_t` 中，写成 `j_t=owner(x_t)`，并默认要求 `j_t != a_t`，不要无来源地引入一个候选 judge 集合。
...
此外，当前 `DecisionEvent` 没有一等的 `o_t` 或 `j_t` 字段；它们目前只能通过
adapter/metadata 传递。若不补充 schema 校验，真实交付物和 judge provenance 无法
可靠重放，八元 peer 模型就仍然只是文档接口。

固定旧任务约束也只能施加在语义未发生变化的回访切片。发生概念反转时，旧任务保持率和新任务适应率可能数学上互相冲突；这时必须报告恢复速度，而不能把旧准确率约束当作普适真理。
此外，实验必须预先规定反馈窗口：决策 horizon 结束后仍在队列中的标签是否只用于
尾部更新、是否计入效用，以及如何处理右删失。否则低延迟方法可能仅因更早收到
反馈而被不公平地比较。
还复核了更新顺序：`OnlineRLSHead.update()` 每条反馈都会按 forgetting 因子衰减，
因此两条合法反馈交换顺序会得到不同参数（示例 theta 差约 `0.02381`）。当前数学
写成无序集合不足以定义这个更新器；实现必须冻结 arrival/source-index 重放顺序，
或改成批量可交换的充分统计量，并加入顺序不变量测试。
