...
| **U-018-decide** ⚠ | **是否加 Multi-Agent Debate (MAD, Liang et al. 2024) 作为第 3 个外部 baseline** — R-FULL-002 reviewer (P3 Adversarial Novelty SAC) **明点 MAD `is_overlap_risk=TRUE`**："TCPB's 'terminal-outcome only' is essentially a degenerate case of MAD's per-hop critique aggregator with aggregator window=full trajectory"；§2.2 partially addressed in text 但 MAD 不在 Table 1 baseline list。两选：(a) **加 MAD 做第 3 个 swap host**（SWAP-4：R2 audit → MAD `final_aggregator`，~+5 d engineer）；(b) **不加**，仅在 §2.2 加一段更详细的 MAD-vs-TCPB 区分 | **(a) 推荐加 MAD**：reviewer 明点 closest-neighbour 必须 benchmarked 才能闭合 D3 novelty 的 overlap_risk cap；MAD swap 与 ChatEval swap 互补（MAD = debate-aggregator，ChatEval = meta-reviewer-aggregator，是两种不同的 per-hop critique 设计）；如选 (b) 则 D3 ≤ 5.5 cap 难以突破 | 解锁 engineer 新工单 E-015 (reproduce MAD baseline) + E-016 (R2 audit swap into MAD aggregator) + scientist S-XXX (§4.x 表加第 3 host 列)；如选 (a)，sprint 时间线净增 ~+5 d engineer (用现有 5-6 d buffer 还能吃掉但变紧) | ⏳ **等用户拍板**（不阻塞当前 R1+R2+R3 + AutoGen+ChatEval 主线）|
...
| **U-019-server-ssh-state-decide** | **Server SSH/clone 状态恢复路径** — engineer Day 6 尝试在 server 上 clone ChatEval (E-010 prep) 时发现：单次 session 内多次卡死 ssh 后产生 7 个 orphaned 进程 + 新连接 `Permission denied (publickey,password)` ❌（与 E-013 ✅ probe 矛盾）。三诊断假设：(1) ssh-agent 状态被 orphaned 进程污染；(2) server fail2ban 临时 ban 本机 IP；(3) server → github 出口慢/被防火墙拦（第一次 clone 卡死 185 s 无任何输出）。详见 `implementation_log.md [E-010_chateval_server_clone_attempt_20260420]` | (b) **用户 ssh 到 server 协助诊断**：`tail -50 /var/log/auth.log`（看是不是被 ban）+ `iptables -L -n`（看是不是有规则）+ `who`（看是不是有别的活跃 session 占用）；如确诊 fail2ban → `sudo fail2ban-client unban <my_ip>`。fallback (a) engineer 等 30 min 自动解锁后重试。最差 (c) 放弃 server 路径，E-010 走 Windows 本地 venv | 阻塞 E-010 server 路径 → 阻塞 E-011/E-012 swap workstream → 阻塞 S-121/S-122/S-123 写作 | ⏳ 待用户拍板 (2026-04-20)；engineer 自挂；high — sprint critical-path 上 |
