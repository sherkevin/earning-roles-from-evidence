...
| **U-014-decide** ⚠ | **外部 baseline 选几个 system 做对比** — reviewer R-FULL-001 fatal #3 直接要求加外部 baseline；当前 paper Table 1 全是 author-internal 变体（详见 [`docs/paper/external_baseline_plan.md`](../paper/external_baseline_plan.md) §1 audit）。三选：(a) **1 system MVP** = AutoGen only (~+5 d engineer)；(b) **2 systems 推荐** = AutoGen + ChatEval (~+9-10 d engineer)；(c) **3 systems aggressive** = AutoGen + ChatEval + MetaGPT (~+14 d engineer，挤掉 R-FULL-002 复审窗口) | **(b) 2 systems 推荐**：AutoGen 是最被引用的 orchestrator baseline，ChatEval 是最干净的 peer-critique baseline；两个一起既覆盖 reviewer fatal #3 又覆盖 fatal #1（Finding 4 自证伪是因为没 hop-level 拦截，正好对应 R2 audit swap 进 ChatEval）；时间在 36-天 sprint 可吸收（用 buffer + 平行 E-006）| 解锁 engineer E-009..E-012 (survey / reproduce / swap / compare) + scientist S-121/S-122/S-123 写作 + 闭合 reviewer R-FULL-001 fatal #3 / S6 cap | ⏳ **等用户拍板**（不阻塞 R1+R2+R3 主线，但显著影响 R-FULL-002 评分上限） |
| **U-015-decide** ⚠ | **module-swap 范围** — 决定我们的 R1 / R2 / R3 各自 swap 进哪个外部 host（不是 full-system 对比，而是"drop-in replacement"，控制变量更严，详见 `external_baseline_plan.md` §2-§4）。三选：(a) **R3 only** = SWAP-1 (R3 vector belief 进 AutoGen `select_speaker`)；(b) **R2+R3 推荐** = SWAP-1 + SWAP-3 (R2 audit 进 ChatEval `MetaReviewer`)；(c) **R1+R2+R3** = +SWAP-5 (R1 split 进 MetaGPT pipeline) | **(b) R2+R3 推荐**：与 U-012 R1+R2+R3 sprint scope 部分对齐（R1 swap 工程量 +5-7 d 不可控，留 future work）；R2 audit swap 是直接反驳 reviewer fatal #1 的关键实验 | 决定 SWAP-1 / SWAP-3 / SWAP-5 哪几个进 paper §4.x 表 | ⏳ **等用户拍板**（强烈建议与 U-014 同步选择）|
| **U-016-decide** | **是否 drop 原 sprint 工单 E-007** — 原 E-007"跑 1-2 个外部系统 4 d full-system 对比"被 [`external_baseline_plan.md`](../paper/external_baseline_plan.md) §5 的 E-009..E-012 完全 subsume（survey + reproduce + swap + compare 流程更严格） | **是 (drop E-007)** — 否则双倍工作量无收益；如保留则 sprint 加 9-10 d，超 deadline | engineer 工单数减 1（E-007 → 替换为 E-009..E-012） | ⏳ **等用户拍板**（与 U-014/U-015 一起决定）|
