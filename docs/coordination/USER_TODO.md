...
| **U-021-decide** ⚠ CRITICAL | **demand.md §2 vs R29 editorial 冲突 — Appendix B/C/D/E 是否符合 8-page rule？** R-FULL-006 P5 strict batch (`reviewer_20260419_221546_06_0297b0`) 引用 demand.md §2 字面：`"All figures, tables, equations, **pseudocode**, and algorithm descriptions must fit entirely within these 8 pages"`，exemption list 仅 `{references, Limitations, Ethical Considerations}` (3 项 exhaustive)。判定 R29 后的 Appendix B (Provider Integrity) + Appendix C (LLM Prompts) + Appendix D (Stage-2 preliminary Table 3) + Appendix E (Algorithm 1 pseudocode) 全 4 个非 exemption Appendix → **DR-1 confirmed → 强制 verdict=reject**。这与你 R29 editorial preference "伪代码不该出现在正文里" 直接冲突。详见 implementation_log `[reviewer_r_full_006_ack_20260420]` (R32 commit 加) | **(c) Hybrid 推荐**：保留 Appendix A (Responsible NLP, 这是 ARR 标准要求) + Limitations；rollback Algorithm 1 (Appendix E) → §3.6 inline (重新压缩 main body)；rollback Appendix D Table 3 → §4.5 inline 1 段 + 1 表（约 8 行）；rollback Appendix B Provider Integrity → §Limitations item (5) 扩段 (回到 R15 状态) 或 §3.x runtime detail；rollback Appendix C LLM Prompts → 仅留 1 句 cross-ref "anonymous code/data supplement"，不在 paper body | (a) Path A "Full rollback all 4": 重 inline 全部 → 严重 main body overflow，需 ~1.5-2 页 main body 压缩 / (b) Path B "Stand pat": 不动，赌 actual ARR reviewer 不会这么 strict literal-read demand.md §2，但 R-FULL-006 BATCH-B 已 strict reject = real risk / (c) Path C Hybrid (推荐) / (d) Path D "rewrite demand.md §2": 修改我们自己的 demand.md §2 把 "Appendix" 加到 exemption list (但这违反 ARR/EMNLP 标准 strict reading) | ⚠ **等用户拍板** — scientist 不擅自 override user editorial (R29) 也不擅自接受 reviewer 严格读法；选定后 scientist 立即执行 rollback + 重压 main body |
