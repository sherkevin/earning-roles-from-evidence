...
| `R-FULL-005` | 全文（待用户触发） | ⏳ 等用户在 USER_TODO §A 加 `U-Review-5-decide`；**强烈建议时点 = experiments_solidity_score 至少 ≥ 4 之后** (即 ≥4 个 EXP 项 pass，例如 +MuSiQue + 多 seed + paired bootstrap + 外部 baseline)。连续 4 轮 weak_reject (overall=4.5) 是结构性信号 — 再做同实验状态的 R-FULL 只会消耗 reviewer 资源不会 unlock score。下一轮 persona 候选：**P4 (Reproducibility-Ethics SAC)** 是唯一未用过的 persona |
