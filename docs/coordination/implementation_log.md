...
- blockers:
  - `experiment.md` ?????????????? `engineer_prompt.md` ?????????????? `implementation_log` + ???????????
- blocker_type:
  - none?????????????????????????????
- verification:
...
- conflicts_NOT_resolved?????????????? U-XXX ???????:
  ...
  - ??????????????????`docs/coordination/SCIENTIST_TODO.md`?????? + ?????????????? + `docs/coordination/implementation_log.md`??????????????????
...
- verification:
  ...
  - lint??N/A???? .tex ??????????? .md lint ????????? implementation_log ????????????? ReadLints??
- impact_on_other_tracks:
...
- key_decisions????????????????????????????:
  ...
  - ??????????`implementation_log.md` ????? phase block + USER_TODO �D L97 + REVIEWER_TODO �D L62???????? `.md` ??????????**???????**???????????????
- blockers:
  - none
- conflicts_with_docs:
  - none
- verification:
  ...
  - `Grep "four-role-todo-workflow\.md|ssh-server-rules\.md"` ??? ??????????????implementation_log + USER �D L97 + REVIEWER �D L62?????????????????????????? `.mdc`
...
- workflow_compliance????? .cursor/rules/three-role-todo-workflow.md �1??:
  ...
  - Step 2 (???????????????? TODO)????? ???? implementation_log ???????????????????????????????TodoWrite ????????? 9 ??????????
- next_action:
...
- key_decisions?????????????????????????????????:
  ...
  - ??? TODO ????????USER_TODO.md??????????/ SCIENTIST_TODO.md????????/ implementation_log.md????????????????????????????
...
- ??????????????cross-doc consistency check??:
  ...
  - `docs/coordination/implementation_log.md`: ????? sprint ???? USER_TODO ???**???????????????** ??? **??????????**
- blockers:
...
- verification:
  ...
  - ReadLints PROJECT_STRUCTURE.md / SCIENTIST_TODO.md / USER_TODO.md / implementation_log.md ??? ?????? no errors
- impact_on_other_tracks:
...
- next_action:
  ...
  - **scientist**: ??????? S-119 (Figure 1 prompt ?????????????)??????? S-115/S-116/S-117/S-118 ?????? blocked on engineer ticket???????"???? + ?????? implementation_log.md"??
...
3. ??? newapi ??????????????????????????? batch ??????????? cost????????????????????? `implementation_log` ???? + ?????? scientist ? `U-EXEC-XXX ??` ???????????
...
3. ??? ???????? E-XXX ??? `implementation_log` append ???????? phase ?????? `[E-001_task_tree_module_20260420]`????? `files_added` / `files_modified` / `commit_ref` / `next_action` / `unblocks_for_scientist`???
...
- next_action:
  - **engineer (any future window)**: ???????????????????????? E-XXX ??????? cross-check C-1 .. C-7 ????? valid??????????? `implementation_log` append ??????????????????????
  - **user**: ???????????
...
- planned_files_modified:
  ...
  - `docs/coordination/implementation_log.md`???????? ??? ??? append result section??
...
- `docs/coordination/implementation_log.md`??????? + 4 ???? phase ????
...
- `docs/coordination/implementation_log.md`??????? + 3 ???? phase ???? ? ? ?????
...
- commit ref: R11 commit??engineer Day 1.5 ????????????????? R11 scientist S-119 commit ???????????????????? implementation_log ?????????
- next_action:
...
- files added: ?????clone ???????????
- files modified: `docs/coordination/implementation_log.md`??????? + E-004 ?????????
...
- `docs/coordination/implementation_log.md`??????? + E-004 ??????? + E-010 ??????? ??? 3 phase ????
...
- commit ref: R12 commit??engineer Day 6 ????????????????? R12 scientist S-120 commit ???????????????????? implementation_log ?????????
- next_action:
...
- `docs/coordination/implementation_log.md`??????? + 6 ???? phase ????
...
- `implementation_log`: ??? ack block (R37 expanded from 1-line placeholder)
...
- `implementation_log`: ??? ack block (R42 expanded R-FULL-008 batch closure)
...
- **Non-LLM follow-ups while waiting for U-EXEC-008**:
  ...
  5. Write a ``docs/paper/r42_engineer_handoff.md`` page so the next engineer session has a single-source handoff instead of grep-crawling implementation_log.
