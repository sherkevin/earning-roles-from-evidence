...
|---|---|
| **What benchmark do we evaluate on?** | HotpotQA (multi-hop QA, English, Wikipedia-sourced) — the canonical benchmark for ALL Stage-1 results in Tables 1, 2, Figure 2, Appendix D. MuSiQue + 2WikiMultiHop are scope-locked for Stage-2 (`U-013` ✅, engineer pipeline). **External baselines currently have HotpotQA smoke evidence at n=50 only** (`MA-RAG` / `MAD` / `ReAgent`), not paper-grade head-to-head rows yet. |
...
| **EX-3** | **Multi-Agent Debate (MAD)** (`composable-models/llm_multiagent_debate`) | Liang et al. 2024 (`liang2023mad`) + Du et al. 2024 (`du2024improving`) | `/media/data3/dengkw/idea04/external_baselines/mad/` ✅ cloned | U-018 ✅ approved (2026-04-20); **smoke done**: HotpotQA-50 with adapter at `artifacts/external_baselines/mad/r41b_n50/metrics.json` gives `EM=0.58`, `F1≈0.7765`, `agents=3`, `rounds=2`; still needs E-016 SWAP-4 + larger comparison batch before scientist can write paper-grade table rows |
...
|---|---|---|---|---|---|
| **EX-4** ⚡ NEW | **MA-RAG** (`thangylvp/MA-RAG`) | Tran et al. 2025 (arXiv:2505.20096) | https://github.com/thangylvp/MA-RAG (15 ⭐) | claims SOTA on HotpotQA + 2WikimQA + MuSiQue with LLaMA3-8B/70B/GPT-4o-mini; 4-agent RAG (Planner/StepDefiner/Extractor/QA); 2025-05 recent | **smoke done** at `artifacts/external_baselines/marag/r41b_n50/metrics.json`: HotpotQA-50, `EM=0.38`, `F1≈0.6478`, Path A gold-context adapter. Still pending engineer E-018 decision whether to push to larger reproduce or keep as smoke-only evidence |
| **EX-5** ⚡ NEW | **ReAgent** (`astridesa/ReAgent`) | Liu et al. EMNLP 2025 (arXiv:2503.06951) | https://github.com/astridesa/ReAgent (4 ⭐) | EMNLP 2025; reversible multi-agent (rollback when conflicts); +6% over forward baselines on HotpotQA + 2 others; conceptually distinct from MAD's debate | **smoke done** at `artifacts/external_baselines/reagent/r41b_n50/metrics.json`: HotpotQA-50, `EM=0.00`, `F1≈0.1800`; current wrapper is explicitly marked format-sensitive in `docs/paper/e018_reagent_adapter_inspection.md`, so this is a *diagnostic smoke*, not a final baseline verdict |
...
| **SS-3** | HotpotQA fullval | **7405** | E-017 (3-seed paired Stage-2 vs Stage-1, on server). **Partial unblock now exists**: `artifacts/round2_gpt41mini_stage2_fullval/paired_stats_seed42_only.csv` gives seed42-only `ΔF1 = +0.0061` with 95% CI `[-0.0016, 0.0139]` and `Δtoken = -360.7` (`840.0 -> 479.3`). This is useful as interim evidence, but does **not** replace the intended 3-seed `paired_stats_3seed.csv`. |
...
| HotpotQA fullval × M-9 vs M-7 paired × 3 seeds (E-017) | 🟡 **partial unblock**: seed42-only paired stats now landed (`ΔF1=+0.0061`, CI crosses 0; `Δtoken=-360.7`); 3-seed final aggregate still pending | `artifacts/round2_gpt41mini_stage2_fullval/paired_stats_seed42_only.csv` + run dirs under `artifacts/round2_gpt41mini_stage2_fullval/` |
...
| MAD reproduce on multi-hop QA (E-015) | 🟡 **smoke done at n=50** (`EM=0.58`, `F1≈0.777`), but larger / swap-ready comparison still pending | `artifacts/external_baselines/mad/r41b_n50/metrics.json` |
| MA-RAG HotpotQA reproduce (E-018) | 🟡 **smoke done at n=50** (`EM=0.38`, `F1≈0.648`) under Path A gold-context adapter; larger reproduce still pending | `artifacts/external_baselines/marag/r41b_n50/metrics.json` |
| ReAgent HotpotQA reproduce (E-018) | 🟡 **diagnostic smoke done at n=50** (`EM=0.00`, `F1≈0.180`), currently wrapper/format-sensitive rather than paper-grade baseline | `artifacts/external_baselines/reagent/r41b_n50/metrics.json` |
